/**
 * Combined Server for Aion Protocol and Quantum Simulator
 * 
 * This Node.js server uses Express to serve both the main Aion Protocol
 * and the Quantum Simulator with Audio Visualization.
 */
...
// Serve the main Aion protocol from dist directory
app.use('/aion', express.static(path.join(__dirname, 'dist')));

// Serve the quantum simulator
app.use('/quantum', express.static(path.join(__dirname, 'quantum-simulator-refined-chat')));

// Serve static files
app.use(express.static(path.join(__dirname, 'public')));

// Main route - redirect to the landing page
app.get('/', (req, res) => {
  res.sendFile(path.join(__dirname, 'landing.html'));
});

// Aion protocol route
app.get('/aion', (req, res) => {
  res.sendFile(path.join(__dirname, 'dist/index.html'));
});

// Quantum simulator route
app.get('/quantum', (req, res) => {
  res.sendFile(path.join(__dirname, 'quantum-simulator-refined-chat/quantum-simulator-complete.html'));
});

// API Routes for the quantum simulator chat
...
app.get('/api/status', (req, res) => {
  res.json({
    ...
    version: '1.1.0',
    components: [
      { name: 'aion-protocol', path: '/aion', description: 'Main Aion Protocol implementation' },
      { name: 'quantum-simulator', path: '/quantum', description: 'Quantum Simulator with Audio Visualization' }
    ],
    timestamp: new Date().toISOString()
  });
});
...
// 404 page handling
app.use((req, res) => {
  res.status(404).sendFile(path.join(__dirname, 'quantum-simulator-refined-chat/404.html'));
});
...
app.listen(PORT, HOST, () => {
  ...
  console.log(`Main Aion Protocol: http://${HOST}:${PORT}/aion`);
  console.log(`Quantum Simulator: http://${HOST}:${PORT}/quantum`);
});